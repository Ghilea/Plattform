	...
	if(empty($error)){ 
		...
		try {
			...
			$database->insert("forum_post",[
				"account_id" => $_SESSION['id'], 
				"forum_thread_id" => $latest_tread_ID,
				"title" => $title,
				"content" => $text,
				"creator" => "1",
				"locked" => $locked,
			...
			if (!empty($_FILES['image']['name'])) {
				...
				$image = $uploadImg->image($latest_tread_ID, $_FILES["image"]["name"], $_FILES["image"]["tmp_name"], "../uploads/");
			
				$database->update("forum_thread",
				["image" => $image],["AND" => ["id" => $latest_tread_ID]]);
			}

			//send location
			$headerMessage = "Location: /pages/forumView.php?id=".$latest_tread_ID."";	
		...
		<div class="content">
			...
			<header>
				<div class="col-full">
					<div class="wrap-col">
						<h2>Skapa tråd</h2>
						<h4>Skriv en rubrik och text. Du kan även lägga till en bild som t.ex. kan visas tillsammans vid nyheter.</h4>
					</div>
				</div>
			</header>
					...
					<div class="col-full">
						<div class="wrap-col">
							...
							<select name="chainLink" id="chainLink">
								<option value="">Koppla samman trådar</option>
